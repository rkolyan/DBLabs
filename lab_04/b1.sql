...
AS $$
	def return_musicians_id_in_musicians(name):
		return plpy.execute("SELECT M.musicians_id FROM musicians AS M WHERE M.name =\'" + name + "\';");
	
	def return_genre_id_in_genres(name):
		return plpy.execute("SELECT G.genre_id FROM genres G WHERE G.name = \'" + name + "\';");
	
	def return_label_id_in_labels(name):
		return plpy.execute("SELECT L.label_id FROM labels L WHERE L.name = \'" + name + "\';");
	...
	albums_ids = plpy.execute("SELECT album_id FROM albums;");
	if (albums_ids == None):
	...
	plpy.execute("INSERT INTO albums (album_id, name, record_year, tracks_count, album_type, musicians_id, label_id, genre_id) VALUES ("+ str(maximum + 1) + ",\'" + str(album_name) + "\',2019,3,\'EP\'," + str(musicians_id) + "," + str(label_id) + "," + str(genre_id) + ");");
$$ LANGUAGE plpython3u;
